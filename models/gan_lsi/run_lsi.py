...
def parse_args():
    ...
    p.add_argument("--algorithm", choices=["mapelites", "cma_es"],
                   default="mapelites", help="Algorithme QD à utiliser")
    p.add_argument("--evaluations", type=int, default=1000,
                   help="Nombre total d’évaluations")
    p.add_argument("--init-pop", type=int, default=100,
                   help="Population initiale (MAP-Elites) ou point de départ (CMA-ES)")
    p.add_argument("--pop-size", type=int, default=20,
                   help="Taille de la population (CMA-ES)")
    p.add_argument("--mutation-power", type=float, default=0.5,
                   help="Écart-type initial de la mutation")
    p.add_argument("--bc1-range", type=float, nargs=2, default=(0.0, 150.0),
                   help="Plage [min max] pour BC1 (n° ennemis)")
    p.add_argument("--bc2-range", type=float, nargs=2, default=(0.0, 25.0),
                   help="Plage [min max] pour BC2 (hauteur moyenne)")
    p.add_argument("--bc1-res", type=int, default=151,
                   help="Résolution de la grille pour BC1")
    p.add_argument("--bc2-res", type=int, default=26,
                   help="Résolution de la grille pour BC2")
    p.add_argument("--java-jar", type=str,
                   default="target/Mario-AI-Framework-jar-with-dependencies.jar",
                   help="Chemin vers le JAR d’évaluation Java")
    p.add_argument("--java-agent", type=str, default="MarioAI-Astar",
                   help="Nom de l’agent Java pour l’évaluation")
    p.add_argument("--temp-txt", type=str, default="temp_level.txt",
                   help="Fichier temporaire de niveau .txt")
    p.add_argument("--out-archive", type=str,
...
def main():
    ...
    while algo.is_running():
        ...
        # a) Génère le JSON & écrit temp_level.txt
        json_lvl = gan_generate(z, args.model_path)
        write_level_txt(json_lvl, args.temp_txt)

        # b) Sauvegarde dans levels/generated/gan/
        gen_dir = os.path.join(os.getcwd(), "levels", "generated", "gan")
        os.makedirs(gen_dir, exist_ok=True)
        fname = f"{ind.ID}.txt"
        shutil.copy(args.temp_txt, os.path.join(gen_dir, fname))

        # c) Évaluation via Java
        cmd = (f"java -cp {args.java_jar} EvaluateLevel "
               f"--input {args.temp_txt} --agent {args.java_agent}")
        out = os.popen(cmd).read()

        # d) Vérifie que la sortie Java est valide
        if "Enemies:" not in out or "AvgHeight:" not in out or "Score:" not in out:
            print("Erreur évaluation Java, sortie was:", out)
            break

        # e) Parse les BC et le score
        bc1   = float(out.split("Enemies:")[1].split()[0])
        bc2   = float(out.split("AvgHeight:")[1].split()[0])
        score = float(out.split("Score:")[1].split()[0])

        # f) Met à jour l’individu
        ind.features  = (bc1, bc2)
        ind.fitness   = score
        ind.statsList = [score]

        # g) Retour à l’algorithme
